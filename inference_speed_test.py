...
class InferenceSpeedTest:
    ...
    def init_model(self):
        self.device = 'cuda'
        self.precision = {'float32': torch.float32,
                          'float16': torch.float16}[self.args.precision]
        self.model = MattingNetwork(self.args.model_variant)
        self.model = self.model.to(
            device=self.device, dtype=self.precision).eval()
        self.model = torch.jit.script(self.model)
        self.model = torch.jit.freeze(self.model)

    def loop(self):
        w, h = self.args.resolution
        src = torch.randn((1, 3, h, w), device=self.device,
        ...
        with torch.no_grad():
            ...
            for _ in tqdm(range(1000)):
                fgr, pha, * \
                    rec = self.model(
                        src, src, *rec, self.args.downsample_ratio)
