...
class MotionAugmentation:
    ...
    def __call__(self, fgrs, phas, pha_coms, bgrs):
        # Foreground affine
        if random.random() < self.prob_fgr_affine:
            fgrs, phas, pha_coms = self._motion_affine(fgrs, phas, pha_coms)

        # Background affine
        if random.random() < self.prob_bgr_affine / 2:
            bgrs = self._motion_affine(bgrs)
        if random.random() < self.prob_bgr_affine / 2:
            fgrs, phas, pha_coms, bgrs = self._motion_affine(
                fgrs, phas, pha_coms, bgrs)

        # Still Affine
        if self.static_affine:
            fgrs, phas, pha_coms = self._static_affine(
                fgrs, phas, pha_coms, scale_ranges=(0.5, 1))
        ...
        pha_coms = torch.stack([F.to_tensor(pha_com) for pha_com in pha_coms])
        bgrs = torch.stack([F.to_tensor(bgr) for bgr in bgrs])

        # Resize
        params = transforms.RandomResizedCrop.get_params(
            fgrs, scale=(1, 1), ratio=self.aspect_ratio_range)
        fgrs = F.resized_crop(fgrs, *params, self.size,
                              interpolation=F.InterpolationMode.BILINEAR)
        phas = F.resized_crop(phas, *params, self.size,
                              interpolation=F.InterpolationMode.BILINEAR)
        pha_coms = F.resized_crop(pha_coms, *params, self.size,
                                  interpolation=F.InterpolationMode.BILINEAR)
        params = transforms.RandomResizedCrop.get_params(
            bgrs, scale=(1, 1), ratio=self.aspect_ratio_range)
        bgrs = F.resized_crop(bgrs, *params, self.size,
                              interpolation=F.InterpolationMode.BILINEAR)
        ...
        if random.random() < self.prob_sharpness:
            ...
            pha_coms = F.adjust_sharpness(pha_coms, sharpness)
            bgrs = F.adjust_sharpness(bgrs, sharpness)

        # Blur
        if random.random() < self.prob_blur / 3:
            fgrs, phas, pha_coms = self._motion_blur(fgrs, phas, pha_coms)
        if random.random() < self.prob_blur / 3:
            bgrs = self._motion_blur(bgrs)
        if random.random() < self.prob_blur / 3:
            fgrs, phas, pha_coms, bgrs = self._motion_blur(
                fgrs, phas, pha_coms, bgrs)

        # Pause
        if random.random() < self.prob_pause:
            fgrs, phas, pha_coms, bgrs = self._motion_pause(
                fgrs, phas, pha_coms, bgrs)
    ...
    def _static_affine(self, *imgs, scale_ranges):
        ...
        imgs = [[F.affine(t, *params, F.InterpolationMode.BILINEAR)
                 for t in img] for img in imgs]
    ...
    def _motion_affine(self, *imgs):
        ...
        angleA, (transXA, transYA), scaleA, (shearXA,
                                             shearYA) = transforms.RandomAffine.get_params(**config)
        angleB, (transXB, transYB), scaleB, (shearXB,
                                             shearYB) = transforms.RandomAffine.get_params(**config)
        ...
        for t in range(T):
            ...
            for img in imgs:
                img[t] = F.affine(img[t], angle, (transX, transY), scale,
                                  (shearX, shearY), F.InterpolationMode.BILINEAR)
        return imgs if len(imgs) > 1 else imgs[0]
    ...
    def _motion_noise(self, *imgs):
        ...
        for img in imgs:
            T, C, H, W = img.shape
            noise = torch.randn((T, 1 if monochrome else C, round(
                H / grain_size), round(W / grain_size)))
    ...
    def _motion_color_jitter(self, *imgs):
        ...
        for t in range(T):
            ...
            for img in imgs:
                img[t] = F.adjust_brightness(img[t], max(
                    1 + lerp(brightnessA, brightnessB, percentage), 0.1))
                img[t] = F.adjust_contrast(img[t], max(
                    1 + lerp(contrastA, contrastB, percentage), 0.1))
                img[t] = F.adjust_saturation(img[t], max(
                    1 + lerp(brightnessA, brightnessB, percentage), 0.1))
                img[t] = F.adjust_hue(img[t], min(
                    0.5, max(-0.5, lerp(hueA, hueB, percentage) * 0.1)))
        return imgs if len(imgs) > 1 else imgs[0]
